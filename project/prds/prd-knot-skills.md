...
- [ ] Cover the full configuration lifecycle: initialise a workspace, create/modify/delete looms and knots, and inspect current state
...
### Story 1: Initialise a Knot Workspace

As a new Knot user, I want to run `knot init` in my project directory so that a workspace folder is created (if not present), Knot is verified to be running, and if Knot is not on my global path it is built and installed before the workspace is initialised.

**Scenarios:**
1. Given I am in a project directory with no workspace folder, when I run `knot init`, then the workspace folder is created relative to the current directory, Knot is checked to be running, and if it is not on the global path it is built and installed, after which I can confirm the setup succeeded
2. Given a workspace folder already exists, when I run `knot init`, then the command confirms the workspace is present and offers to show its current state instead of recreating it
...
**Scenarios:**
1. Given I have a workspace, when I describe a new loom (source directory, target, knot definitions), then a skill calls Knot's HTTP API to create the loom and its knots and I receive confirmation
...
### Story 3: Inspect Workspace Status

As a user, I want to see the current state of my Knot workspace — which looms exist, what knots are configured, and their processing status — so I can understand what is set up and whether it is working correctly.

**Scenarios:**
1. Given a configured workspace, when I ask to inspect my workspace, then a skill calls Knot's HTTP API and presents a summary of looms, knots, and their state
...
- [ ] A user can inspect any aspect of their workspace state through the `knot-inspect` skill

## Dependencies & Constraints

- Technical: Knot must expose HTTP endpoints covering workspace init, loom CRUD, knot CRUD, and status inspection — these are prerequisites for the skills
