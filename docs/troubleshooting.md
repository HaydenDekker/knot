...
| `TurnContinued` (Knot 0.47.0+) | **Not an error.** A compaction landed between the agent's last turn and its final answer, so the settled turn had no text; Knot sent one follow-up `prompt` on the live session ("reply with your final answer now") and the run continued **in the same process**. `reason=` names the compaction that ended the turn | Nothing — this is the cheap healthy path (the in-session sibling of `SessionRestarted`, which needs a new process). It is fire-once per attempt: a `TurnContinued` followed by `KnotEmptyResponse` means pi did not answer the follow-up either, and the ordinary session-resume nudge takes over | 
| `RunAbandoned` (Knot 0.47.0+) | Emitted **at startup**, once per queue entry restored from the previous service: the entry's file survived, so the run that picked it up stopped mid-strand (service stopped, crash, OOM kill). `session=` is absent because the session id died with the process | Expect a fresh `KnotProcessing` for the same strand right after — that is a **re-run from scratch**, not a continuation (Knot never re-enters a dead agent session), so uncommitted work from the abandoned run may be repeated. If a strand is abandoned repeatedly, look for what kills the service (`dmesg`, the shell that launched it) rather than at the strand | 
| CompactionInterrupted (Knot 0.46.0+; any reason since 0.47.0) | A compaction **started but never reported completion** (`compaction_start` with no `compaction_end`, no terminal `errorMessage`) — pi died mid-summarisation, leaving the context unshrunk. Since 0.47.0 this counts for **any** `reason`, `threshold` included: Knot no longer stops the process it is watching (it closes stdin on pi's `agent_settled`, and holds it open for a whole compaction span), so an unclosed span is a **pi-side** death — Knot is no longer a cause. This is **resumable** (it carries the session id), not the terminal `context limit reached`. Knot records `CompactionInterrupted` and attempts an **out-of-band manual compact** (re-opens the same session with `--session <id>` + a `compact` RPC) — bounded to one per failed execution | Watch the recovery chain in the service log: `CompactionInterrupted` → `ManualCompactionSucceeded` + `SessionRestarted` (recovered — the strand re-enters and continues) is the good shape. A `ManualCompactionFailed` after the `CompactionInterrupted` means the explicit compact also could not shrink the context — the run is terminal; narrow the prompt/strand scope or use a larger-window model as above. Repeated `CompactionInterrupted` on one strand = the context is too large *and* the in-process recovery keeps dying mid-turn — first narrow the scope, then check for the kill cause (inactivity / a wrapping shell dropping the RPC process) |
| ContextCompactionFailed (Knot 0.45.0+) | A compaction span ended without success: pi reported an `errorMessage` (`error=…` in the service log) or the compaction was aborted (`aborted=true`) — e.g. the compact-and-retry itself hit the overflow a second time. The strand continues (the session is still live); this entry is the visible record of a failed recovery that was previously silent | Read the `error=` / `aborted=` fields in the `ContextCompactionFailed` service-log line for pi's diagnosis. Repeated `reason: "overflow"` spans on one strand = the scope is too large — narrow the prompt/strand scope as above. An `aborted=true` without an error usually means the span was cut short (interrupt / inactivity), not a context problem |
| pi auto-compaction is disabled (startup warning, Knot 0.45.0+) | At startup Knot self-heals the project's `.pi/settings.json` (merges `compaction.enabled: true` when the global pi setting disables it) — the warning means it did **not** enable it: an explicit project-level `compaction.enabled: false` (operator opt-out — honoured), an unparseable project settings file (left untouched), or a write failure. The reason clause names which | For an opt-out this is expected — overflow recovery degrades to `context limit reached` failures; re-enable by writing `{"compaction": {"enabled": true}}` to the project `.pi/settings.json`. For the other two: fix/replace the malformed project settings file or fix the permissions, then restart the service |
| context exhausted mid-work, nothing committed | A long run burned its context without stopping to commit or note the incomplete work, so the session died leaving an uncommitted working tree | Enable graceful completion (Knot 0.42.0+): set `agent-adapter: pi-rpc` in `rig/.workspace-agent-config.yaml` **and** a per-alias `ctx-wrap-up-limit` (a few thousand tokens below the model's compaction point) in `rig/models.yml`. Knot then steers the agent to commit + note the incomplete and tie off before the limit. Check `ContextWrapUpSteered` entries in the service log to confirm the steer fired |
...
- **A compacting session is not a stalled one** (Knot 0.47.0+): pi emits no
  stream bytes while a compaction summarises, so a compaction span now counts
  as activity and the inactivity watchdog stands down for its duration. On
  0.47.0+ an `AgentInactivity` whose `silent=` covers a compaction means the
  span had already closed — and a compaction that truly wedges is killed on
  the **total budget** instead (a `Timeout`, not an `AgentInactivity`).
  Before 0.47.0 a long compaction on a loaded machine could be killed as a
  stall at 300s.
