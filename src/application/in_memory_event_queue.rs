...
mod tests {
    ...
    /// with no armed waiter is fine — the event is in memory and the
    /// `front()` peek sees it; the *next* push is caught by the
    /// re-armed permit.)
    #[tokio::test]
    async fn armed_notified_dropped_on_front_hit_is_harmless() {
        ...
        // stays in the queue (visible to the front() peek).
        queue.push(make_pending(created("file-b.md")));
    ...
    /// is still visible to `front()` — the scan half of the
    /// arm-before-check guarantee (the wake half only covers pushes
    /// after the arm).
    #[test]
    fn push_before_arm_is_still_visible_to_front() {
        ...
        // Arm after the push; front() must still see the event.
