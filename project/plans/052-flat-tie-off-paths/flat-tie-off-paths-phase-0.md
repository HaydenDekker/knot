...
- [x] Update doc comment on `compute_tie_off_path()` — "Uses statically derived path: `rig/tie-offs/{loom-id}/tie-off-{knot-name}.md`"
...
- `process_strand_deleted_includes_strand_history` test (in `process_strand.rs`) also had a hardcoded nested tie-off path (`/rig/tie-offs/test-loom/k1/tie-off-k1.md`) that needed flattening. This was a Phase 1 item but was pulled forward because it blocked `cargo test --lib` verification.
